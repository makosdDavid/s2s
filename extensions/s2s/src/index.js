...
register(({ analytics, browser, init, settings }) => {
  ...
  const CONFIG = {
    ...
    SERVER_ENDPOINT: settings.serverEndpoint || 'https://tomcsanyi.eu/api/ga4/collect'
  };
  ...
  function handleShopifyEvent(event) {
    ...
    // Send to server
    sendEventToServer(event.name, formattedEvent);
  ...
  function sendEventToServer(eventName, eventData) {
    try {
      ...
      .then(response => {
        if (CONFIG.DEBUG) {
          console.log(`Event ${eventName} sent to server with status: ${response.status}`);
        }
      })
      .catch(error => {
        if (CONFIG.DEBUG) {
          console.error(`Error sending event ${eventName} to server:`, error);
        }
      });
    } catch (error) {
      if (CONFIG.DEBUG) {
        console.error('Error preparing event for server:', error);
      }
    }
  }
